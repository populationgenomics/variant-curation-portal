...
def get_gnomad_lof_variants(gnomad_version, gene_ids, include_low_confidence=False, annotate_caf=None, flag_curated=False):
    ...
    # Annotate with CAF estimates.
    if annotate_caf is not None:
        # Load the passed constraint table.
        constraint = hl.read_table(annotate_caf)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "—-annotate-caf",
        type=str,
        help="Annotate the variants with cumulative allele frequency estimates from the provided hail constraint table. Expects tables of the format generated by target_constraint.py",
    )
    parser.add_argument(
        "--flag-curated",
        action="store_false",
        help="Flag variants that have already been curated",
    )
    parser.add_argument(
        "--output",
        required=True,
